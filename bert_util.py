...
def calculate_token_probabilities(args : BatchArgs, inputs_embeds : torch.Tensor):
    ...
    masked_token_probs = probs[torch.arange(len(probs)), args.mask_positions, :]
...
def get_token_distribution(sentences : list[str]):
    ...
    mask_positions = [ input_ids.tolist().index(tokenizer.mask_token_id) for input_ids in tokenized['input_ids']]
    mask_positions = torch.tensor(mask_positions, dtype=torch.long)
    probs = model_result.logits[torch.arange(len(sentences)), mask_positions, :].softmax(dim=1)
